...
export function quot(dividend, divisor) {
  ...
  return Math.trunc(div(dividend, divisor));
}
...
export function mod(dividend, divisor) {
  ...
  if (lt$(mod, 0)) {
    mod = lt$(divisor, 0) ? sub(mod, divisor) : add(mod, divisor);
  }
  return mod;
}
...
export function lt$(...rest) {
  ...
  return Boolean(
    rest.reduce((acc, cur) => {
      return eq(acc, false) ? acc : lt$(acc, cur);
    })
  );
}
...
export function gt$(...rest) {
  ...
  return Boolean(
    rest.reduce((acc, cur) => {
      return eq(acc, false) ? acc : gt$(acc, cur);
    })
  );
}
...
export function lte$(...rest) {
  spec({ func: "lt", spec: { isNumberSet: isNumberSet(rest) } });
  return rest.reduce((acc, cur) => {
    return eq(acc, false) ? acc : lte$(acc, cur);
  });
}
...
export function gte$(...rest) {
  spec({ func: "gt", spec: { isNumberSet: isNumberSet(rest) } });
  return rest.reduce((acc, cur) => {
    return eq(acc, false) ? acc : gte$(acc, cur);
  });
}
...
export function float$(n) {
  return and(eq(Number(n), n), notEq(remain(n, 1), 0));
}
